...
function scrapeAllTweets(screen_name) {
	var outfile = csv.csvUpdateStream('../output/'+screen_name, Constants.KEY_OBJ);
	scrapeAllTweets_helper(screen_name, outfile);
}

function scrapeAllTweets_helper(screen_name, outfile, maxTweetId) {
	var options = {};
	...
	requestObject.callback = function(err, reply) {
		...
		if (reply.length > 0) {
			...
			scrapeAllTweets_helper(screen_name, outfile, maxTweetId);
		} else {
